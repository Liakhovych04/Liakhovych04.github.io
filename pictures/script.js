...
const accordion = triggerSelector => {
  ...
  // Вариант с использованием классов в CSS
...
const forms = () => {
  ...
  // Если при загрузке имя файла слишком длинное и чтоб не поехала верстка
  upload.forEach(item => {
...
const modals = () => {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      // Условие для проверки, если хотя бы одна модалка открыта, то автоматически через 60 секунд она не показывается
      let display;
      ...
      // Если ни одно модальное окно не показывается, то мы показываем вот это
      if (!display) {
  ...
  // Функция для того чтоб не прыгала страница при открытии модального окна
  function calcScroll() {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.feedback-slider-item', 'horisontal', '.main-prev-btn', '.main-next-btn');
